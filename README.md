...
<img src="assets/spotnik-logo.png" alt="Spotnik Logo" width="350">

</div>

# 🛰️ Spotnik – AzuraCast Spotify Downloader

Spotnik is a plugin + service for AzuraCast that enables users to download tracks from Spotify using spotDL.

## Overview

It consists of:

- **AzuraCast plugin (PHP)** to trigger downloads
- **Isolated Docker microservice (Python)** that wraps spotDL and exposes a secure internal API

## Features

- Download Spotify tracks via playlist, album, or URL
- Tight integration with AzuraCast's UI and config
- Secure internal-only access (not publicly exposed)
- Designed for maintainability and modularity

## Disclaimer

This project uses [spotDL](https://github.com/spotDL/spotify-downloader), which downloads music from YouTube based on Spotify playlists or tracks.

We don't support or encourage downloading copyrighted music without permission.  
You're responsible for following the law and platform rules when using this tool.
